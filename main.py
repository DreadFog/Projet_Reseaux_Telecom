...
if __name__ == "__main__":
    ...
    for i in range(1, len(liste_CA) -1): # omit first and last
        liste_CA[i].ajouterVoisin([(liste_CA[(i-1)%len2].adresse, list(), ca_wire, liste_CA[(i-1)%len2]), # connected to 2 CA
                                    ...
                                    (liste_CTS[(i+1)%len1], list(), cts_ca_wire, liste_CTS[(i+1)%len1]), # connected to 3 CTS
                                    (liste_CTS[i%len1], list(), cts_ca_wire, liste_CTS[i%len1]),
                                    (liste_CTS[(i-1)%len1], list(), cts_ca_wire, liste_CTS[(i-1)%len1])])  
    # create the users
    ...
    flattened_list_user = flatten(liste_users)
    for client in flattened_list_user:
        ...
        while (client_dest == client):
            client_dest = flattened_list_user[random.randint(0, len(flattened_list_user)-1)]
        if not(client.appel(client_dest.adresse)):
            nb_appels_refuse += 1
    print(f"Nombre total d'appels refusés : {nb_appels_refuse}")
else:
